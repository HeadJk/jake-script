...
function parser(tokens) {
    ...
    const parseNumericLiteral = (token) => {
        current++;
        return new types_1.NumericLiteralNode({ value: token.value });
    };
    const parseCallExpression = (token) => {
        ...
        current++;
        return new types_1.CallExpressionNode({ identifier, argument });
    };
    const parseBinaryExpression = (token, next) => {
        ...
        current++;
        const right = parse();
        return new types_1.BinaryExpressionNode({ left, right, operator });
    };
    const parse = () => {
    ...
    while (current < tokens.length) {
        program.body.push((0, types_1.serialize)(parse()));
    }
    return program;
}
